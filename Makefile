...
maincut-whisper-loudness-normalize.wav: maincut.wav maincut-loudness-seek.rc
	. ./maincut-loudness-seek.rc ; \
	ffmpeg \
		-loglevel info \
		-ss $${seek_start} -to $${seek_end} -i maincut.wav \
...
msg-${date}.mp3: maincut-whisper-loudness-normalize.wav
	ffmpeg -i $< .$@
	mv .$@ $@
